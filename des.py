    ...
    def run(self, seed=None):
        ...
        while self.events_queue:
            ...
            if process_to_run.process_state != ProcessStates.READY:
                raise ValueError(
                    "Process " + str(process_to_run) + " not in runnable state (" + str(
                        process_to_run.process_state) + ").")
            ...
            if ret_event.event_type != EventTypes.PROC_CPU_DONE:  # 如果新事件不是完成态，那么就要加入到事件队列里
